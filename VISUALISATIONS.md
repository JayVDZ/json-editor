# Multiple Visualisation Types - Implementation Plan

## Current State

The application currently has a basic **hierarchical tree graph** visualisation:
...
- **Features to implement**:
  ...
  - ✅ Zoom and pan
  - ✅ Colour-coded by type (object, array, primitive)
...
- **Features to implement**:
  ...
  - ✅ Colour-coded by type and depth
...
- **Usage**: Nearly all advanced visualisations can use D3
...
2. **Create visualiser architecture**
   - Abstract base class or interface for visualisers
   - Each visualiser implements: `render(json)`, `clear()`, `destroy()`
   - Shared container in visualisation panel

3. **Add visualisation switcher UI**
   - Dropdown or button group in visualisation panel header
   - Options: "Graph", "Force Graph", "Treemap", "Tree View", "Table"
   - Persist selection in localStorage
   - Show/hide relevant controls per visualiser

### Phase 2: Implement Priority Visualisations
...
### Phase 3: Polish & UX
1. Add visualisation-specific controls (zoom, layout options)
2. Smooth transitions when switching visualisations
3. Handle edge cases (very large JSON, deeply nested structures)
4. Performance testing with large datasets
5. Add tooltips and help text

### Phase 4: Additional Visualisations (Optional)
1. Implement remaining visualisations based on user feedback
2. Consider usage analytics (if offline) to prioritise

---

## Code Structure Recommendations

### Visualiser Base Pattern
```javascript
class BaseVisualiser {
  ...
  render(json) {
    // Parse JSON and render visualisation
  }
...
class D3ForceGraphVisualiser extends BaseVisualiser {
  render(json) {
...
```javascript
const visualisers = {
  'graph': new GraphVisualiser(container),
  'force': new D3ForceGraphVisualiser(container),
  'treemap': new D3TreemapVisualiser(container),
  'tree': new TreeViewVisualiser(container),
  'table': new TableViewVisualiser(container)
};

let currentVisualiser = 'graph';

function switchVisualiser(type) {
  visualisers[currentVisualiser].clear();
  currentVisualiser = type;
  visualisers[type].render(currentJSON);
  localStorage.setItem('visualiser', type);
}
```
...
1. At least 4 visualisation types are implemented and working
2. User can switch between visualisations seamlessly
3. All visualisations work completely offline
4. Selection persists across page reloads
5. Performance is acceptable for typical JSON files (< 10MB)
6. All visualisations follow British English spelling conventions
...
- Current implementation is in `index.html` with `GraphVisualiser` class
- Graph visualisation panel is on the right side, editor on left
- JSON is validated before visualisation renders
- Use `visualiseJSON()` function as entry point
